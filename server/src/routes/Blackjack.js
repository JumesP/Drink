...
router.post("/game", (req, res) => {
    ...
    const structureOfGame = { // for mental
        dealer: [{ value: "A", suit: "spades" }, { value: "2", suit: "clubs" }],
        players: {
            names: ["Jimbob", "Liam", "Sienna", "Dylan"],
            hands: [
                {
                    owner: "Jimbob",
                    hand: [{ value: "A", suit: "spades" }, { value: "2", suit: "clubs" }],
                    total: 13,
                    status: "In Play"
                },
                {
                    owner: "Liam",
                    hand: [{ value: "K", suit: "hearts" }, { value: "Q", suit: "diamonds" }],
                    total: 20,
                    status: "Stand"
                },
                {
                    owner: "Sienna",
                    hand: [{ value: "5", suit: "hearts" }, { value: "5", suit: "diamonds" }],
                    total: 10,
                    status: "In Play"
                },
                {
                    owner: "Dylan",
                    hand: [{ value: "10", suit: "clubs" }, { value: "J", suit: "spades" }],
                    total: 20,
                    status: "Bust"
                }
            ]
...
router.get("/generateHand", (req, res) => {
    ...
    return { Owner: "Hand Owner", Hand: PairOfCards.getHand(), Total:PairOfCards.handTotal() };
});
...
router.post("/dealHands", (req, res) => {
    ...
    for (let i = 0; i < players.length; i++) {
        ...
        let Player = new BJPlayer(players[i], PairOfCards.getHand(), PairOfCards.handTotal(), status = "In Play");
    ...
    let dealerHand = new BJDealer( "Dealer", PairOfCards.getHand(), PairOfCards.handTotal() );
